...
const UPCOMING_REWARD_PERIOD_MESSAGES = [
...
export function getRandomRewardPeriodMessage(
  distributionAmount: bigint,
  startTime: Date,
  endTime: Date,
): string {
  const randomIndex = Math.floor(Math.random() * UPCOMING_REWARD_PERIOD_MESSAGES.length)
  const message = UPCOMING_REWARD_PERIOD_MESSAGES[randomIndex]
