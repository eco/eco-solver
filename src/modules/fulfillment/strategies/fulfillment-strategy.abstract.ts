        ...
        try {
          ...
          const validationPromises = validations.map(
            async (validation): Promise<ValidationResultWithFee> => {
              const validationName = validation.constructor.name;
              return this.otelService.tracer.startActiveSpan(
                `quote.validation.${validationName}`,
                {
                  attributes: {
                    'validation.name': validationName,
                    'intent.hash': intent.intentHash,
                    ...
                    if (result) {
                      ...
                      if (this.isFeeCalculationValidation(validation)) {
                        await this.otelService.tracer.startActiveSpan(
                          `quote.calculateFee.${validationName}`,
                          {
                            attributes: {
                              'quote.fee.validation': validationName,
                            },
                          },
                          async (feeSpan) => {
                            try {
                              feeResult = await validation.calculateFee(intent, context);
                              if (feeResult) {
