...
export class CCTPProviderService implements IRebalanceProvider<'CCTP'> {
  ...
  async execute(walletAddress: string, quote: Quote<'CCTP'>) {
    this.logger.debug(
  ...
  private _execute(walletAddress: string, quote: Quote<'CCTP'>) {
  ...
  private async executeWithKernel(walletAddress: string, quote: Quote<'CCTP'>) {
