...
| `ccip-abis.ts`                         | Router/on-ramp ABIs and the off-ramp `ExecutionStateChanged` event definition used by the custom client.                                                          |
| `../jobs/check-ccip-delivery.job.ts`   | BullMQ job that polls the destination router/off-ramp until the CCIP message reaches a success/failure state, updating rebalance records accordingly.             |
| `../queues/liquidity-manager.queue.ts` | Exposes `startCCIPDeliveryCheck(...)` so the provider can enqueue delivery checks with consistent retry/backoff behavior.                                         |
...
interface CCIPConfig {
  ...
  delivery?: {
    // Optional overrides for delivery polling
...
To expose CCIP externally, add `'CCIP'` to the wallet’s `walletStrategies` list inside the
`liquidityManager` config block and flip `ccip.enabled` to `true`.
