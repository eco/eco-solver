...
export class LmTxGatedKernelAccountClientService {
  ...
  async getClient(chainId: number) {
    ...
    const wallet = client.kernelAccount?.address
    if (!wallet) {
      throw new Error('LmTxGatedKernelAccountClientService: No wallet address found on client')
    }
        ...
        if (prop === 'execute') {
          return async (calls: Array<{ to: string; data?: string; value?: bigint }>) =>
            this.txQueue.enqueue(wallet, chainId, () => {
              const exec = (target as any).execute
              if (typeof exec !== 'function') {
                throw new TypeError('Kernel client: execute is not a function')
              }
