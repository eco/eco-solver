...
describe('LmTxGatedKernelAccountClientV2Service', () => {
  ...
  it('throws error when client has no wallet address', async () => {
    ...
    const wrapper = new LmTxGatedKernelAccountClientV2Service(underlying as any, {} as any)

    await expect(wrapper.getClient(1)).rejects.toThrow(
      'LmTxGatedKernelAccountClientV2Service: No wallet address found on client',
    )
  })
