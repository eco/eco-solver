...
describe('CheckCCIPDeliveryJobManager', () => {
  ...
  it('throws an unrecoverable error when transfer fails permanently', async () => {
    mockCcipClient.getTransferStatus.mockResolvedValue('Failure')
  ...
  it('delays the job when transfer is still pending', async () => {
    mockCcipClient.getTransferStatus.mockResolvedValue('InProgress')
    ...
    expect(job.updateData).toHaveBeenCalledWith(expect.objectContaining({ pollCount: 1 }))
  ...
  it('fails when pending after exhausting poll attempts', async () => {
    ...
    expect(job.updateData).toHaveBeenCalledWith(expect.objectContaining({ pollCount: 2 }))
