...
describe('WrappedTokenService', () => {
  ...
  describe('getWrappedTokenRebalances', () => {
    ...
    it('should fetch chains from inboxTokens and process each chain', async () => {
      ...
      const getWETHRebalanceSpy = jest
        .spyOn(wrappedTokenService as any, 'getWETHRebalance')
      ...
      expect(getWETHRebalanceSpy).toHaveBeenCalledWith(checksummedKernelAddress, 1, token1)
      expect(getWETHRebalanceSpy).toHaveBeenCalledWith(checksummedKernelAddress, 10, token2)
    })
  ...
  describe('getWETHRebalance', () => {
    ...
    it('should handle token data retrieval failure', async () => {
      ...
      balanceService.getAllTokenDataForAddress = jest
        .fn()
        .mockRejectedValue(new Error('Token data retrieval failed'))
    ...
    it('should handle quote retrieval failure', async () => {
      ...
      liquidityProviderService.getQuote = jest
        .fn()
        .mockRejectedValue(new Error('Quote retrieval failed'))
