...
export class SolanaWalletFulfillService implements IFulfillService {
  ...
  private isSPLTokenTransfer(call: CallDataInterface): boolean {
    ...
    if (
      targetString === tokenProgramId ||
      targetString.toLowerCase() === tokenProgramId.toLowerCase()
    ) {
  ...
  private async createSPLTransferInstruction(
    ...
    try {
      ...
      console.log(
        'MADDEN: createSPLTransferInstruction destinationTokenAccount',
  ...
  private async createFulfillInstruction(
    ...
    const [executorPda] = web3.PublicKey.findProgramAddressSync([EXECUTOR_SEED], portalAddress)
  ...
  private async getTokenTransferAccounts(
    ...
    console.log(
      ...
      route.tokens.length,
      '* 3 (VEC_TOKEN_TRANSFER_ACCOUNTS_CHUNK_SIZE)',
    )
    ...
    console.log(
      'MADDEN: getTokenTransferAccounts - total accounts:',
      accounts.length + callAccounts.length,
    )
    console.log(
      'MADDEN: getTokenTransferAccounts - token accounts:',
      accounts.map((acc) => acc.pubkey.toString()),
    )
    console.log(
      'MADDEN: getTokenTransferAccounts - call accounts:',
