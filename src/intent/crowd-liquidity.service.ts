...
export class CrowdLiquidityService implements OnModuleInit, IFulfillService {
  ...
  isRouteSupported(intentModel: IntentSourceModel): boolean {
    const isSupportedReward = intentModel.intent.rewardTokens.every((token) => {
      return this.isSupportedToken(Number(intentModel.event.sourceChainID), token)
    })
    const isSupportedRoute = intentModel.intent.targets.every((target, index) => {
      const areSupportedTargetTokens = this.isSupportedToken(
        Number(intentModel.intent.destinationChainID),
        target,
      )
      const isSupportedAction = this.isSupportedAction(intentModel.intent.data[index])
