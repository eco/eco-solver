...
export class LiquidityManagerLogger extends BaseStructuredLogger {
  ...
  log(context: LiquidityManagerLogContext, message: string, properties?: object): void {
    const structure = EcoLogMessage.forLiquidityOperation({
      ...
      currentBalanceIn: context.currentBalanceIn,
      targetBalanceIn: context.targetBalanceIn,
      currentBalanceOut: context.currentBalanceOut,
      targetBalanceOut: context.targetBalanceOut,
      tokenInDecimals: context.tokenInDecimals,
      tokenOutDecimals: context.tokenOutDecimals,
