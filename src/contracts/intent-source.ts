...
// Define the type for the Withdrawal event in the contract
export type WithdrawalEventViemType = Prettify<
  GetEventArgs<
    typeof IntentSourceAbi,
    'Withdrawal',
    {
...
/**
 * Define the type for the Withdrawal event log
 */
export type WithdrawalEventLog = DecodeEventLogReturnType<typeof IntentSourceAbi, 'Withdrawal'>

// Define the type for the Withdrawal event log
export type WithdrawalLog = Prettify<
  Log<bigint, number, false, ExtractAbiEvent<typeof IntentSourceAbi, 'Withdrawal'>, true> & {
    sourceNetwork: Network
    sourceChainID: bigint
  }
>

export function decodeWithdrawalLog(data: Hex, topics: [signature: Hex, ...args: Hex[]] | []) {
  return decodeEventLog({
    abi: IntentSourceAbi,
