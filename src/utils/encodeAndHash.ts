        ...
        // Use Anchor's BorshCoder for proper Solana serialization
            ...
            const encoded = svmCoder.types.encode('Route', {
                ...
                calls: calls.map(({ target, data, value }, callIndex) => {
                    ...
                    console.log(`encodeRoute2: using accounts ${startIndex} to ${startIndex + accountCount - 1}`)
                    console.log(`encodeRoute2: call accounts:`, callAccounts.map(acc => acc.pubkey.toString()))
                    ...
                    const calldataAccountCountByte = Buffer.from([calldataWithAccounts.calldata.account_count])
                    
                    const calldataBytes = Buffer.concat([calldataDataLength, calldataDataBytes, calldataAccountCountByte])
                    ...
                    const accountsData = Buffer.concat(calldataWithAccounts.accounts.map(acc => {
...
export function hashIntentSvm2(destination: bigint, route: RouteType, reward: RewardType, accounts: web3.AccountMeta[]): {
    routeHash: Hex
    rewardHash: Hex
    intentHash: Hex
  } {
