...
// import type { RedlockRedisClient } from "redlock" // Commented due to import issues
import { EcoError } from '../errors'
import { EcoLogMessage } from '../logging'
import { RedisConfig } from "../types/config.types"
import { QueueMetadata } from '../messaging/queue-constants'
...
export class RedisConnectionUtils {
  ...
  static getClientsForRedlock(redisConfig: RedisConfig): any[] {
    const connection = redisConfig.connection
    if (this.isClusterConnection(connection)) {
      return [
        new Redis.Cluster(
