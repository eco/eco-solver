...
// TODO: These intent utilities should be properly exposed from the domain library
// Currently the fee service imports from '@lib/intent/utils' but this module doesn't exist
// The actual implementation is in apps/intent-engine/src/domain/utils.ts
// For testing, we create inline mocks for now until the library structure is fixed
...
jest.mock('domain', () => {
  ...
  return {
    ...
    getFunctionCalls: jest.fn(),
    getFunctionTargets: jest.fn(),
    getNativeCalls: jest.fn(),
    isNativeIntent: jest.fn(),
  }
})
