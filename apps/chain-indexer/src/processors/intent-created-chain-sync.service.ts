...
import { MultichainPublicClientService, EcoConfigService } from '@libs/integrations'
import { ChainSyncDomainService } from '@libs/domain'
import { IntentCreatedLog, EcoLogMessage, IntentSource, IntentSourceAbi, IntentSourceModel } from '@libs/shared'
import { WatchCreateIntentService } from '../listeners/intent/watch-create-intent.service'
...
export class IntentCreatedChainSyncService extends ChainSyncService {
  ...
  constructor(
    ...
    readonly multichainClientService: MultichainPublicClientService,
  ...
  async getMissingTxs(source: IntentSource): Promise<IntentCreatedLog[]> {
    const client = await this.multichainClientService.getClient(source.chainID)
