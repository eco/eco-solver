...
import { MultichainPublicClientService, EcoConfigService, EcoAnalyticsService, IntentSource, ERROR_EVENTS } from '@libs/integrations'
import { IntentCreatedLog, EcoLogMessage, ChainIndexerJobUtils, serialize } from '@libs/shared'
  ...
  addJob(source: IntentSource): (logs: Log[]) => Promise<void> {
    return async (logs: IntentCreatedLog[]) => {
      ...
      for (const log of logs) {
        ...
        const createIntent = serialize(log)
        ...
        try {
          ...
          await this.intentQueue.add('process-intent-created', createIntent, {
            jobId,
            ...this.watchJobConfig,
          })
