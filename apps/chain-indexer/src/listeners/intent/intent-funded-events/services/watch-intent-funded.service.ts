...
import { IntentFundedLog, EcoLogMessage, ChainIndexerJobUtils, IntentSource, IntentSourceAbi, ERROR_EVENTS } from '@libs/shared'
...
export class WatchIntentFundedService extends WatchEventService<IntentSource> {
  ...
  constructor(
    @InjectQueue('source-intent') protected readonly intentQueue: Queue,
    ...
    private readonly intentValidationService: IntentValidationService,
  ...
  private async isOurIntent(log: IntentFundedLog): Promise<boolean> {
     ...
     */
    const isOurIntent = await this.intentValidationService.validateIntentOwnership(log)
  ...
  addJob(source: IntentSource, opts?: { doValidation?: boolean }): (logs: Log[]) => Promise<void> {
    return async (logs: IntentFundedLog[]) => {
      for (const log of logs) {
        ...
        const jobId = ChainIndexerJobUtils.getIntentJobId('watch-intent-funded', intentHash, intentFunded.logIndex)
        ...
        try {
          ...
          await this.intentQueue.add('validate-intent', intentHash, {
            jobId,
            ...this.watchJobConfig,
          })
